    ...
    if slots > 0 and actual_idr_balance >= config.MIN_ORDER_IDR * 1.5 and not is_bear:
        ...
        if candidates:
            ...
            for c in candidates:
                ...
                if regime_info.get("hmm_confidence", 0) > 0.6:
                    score += 1
                rsi = sig.get("rsi")
                if rsi is not None and rsi < 55:
                    score += 1
                vr = sig.get("volume_ratio")
                if vr is not None and 0.5 <= vr <= 3.0:
                    score += 1
                atr = c.get("atr")
                if atr is not None and atr < 8:
                    score += 1
                streak = sig.get("momentum_streak")
                if streak is not None and abs(streak) < 3:
                    score += 1
                ema50 = c.get("ema50")
                ...
                if len(ohlcv_p) >= 15:
                    ...
                    if pp > 75 or pp < 25:
                        print(f"  Range filter: {c['pair']} pp={pp:.0f}% {'>75 (puncak)' if pp > 75 else '<25 (jatuh)'} — skip", flush=True)
                        continue
        ...
        per_bin = max(25000, int(actual_idr_balance * (config.MAX_POSITION_PCT_PER_ASSET * config.MAX_OPEN_POSITIONS) / max(n_bins, 1)))
